...
interface ComparisonData {
  temperature: { current: number; previous: number };
  windSpeed: { current: number; previous: number };
  cloudCoverage: { current: number; previous: number };
  uvIndex: { current: number; previous: number };
}
...
const ComparisonCard: React.FC<ComparisonCardProps> = ({ data }) => {
    ...
    return (
      <div className="flex items-center justify-between p-3 bg-white/10 rounded-lg">
        ...
        <div className="flex items-center space-x-2">
          ...
          {diff !== 0 && (
            <div className={`flex items-center space-x-1 ${isIncrease ? 'text-red-300' : 'text-green-300'}`}>
  ...
  return (
    <div className="bg-white/20 backdrop-blur-md rounded-2xl p-6 border border-white/20 shadow-lg">
      <h3 className="text-xl font-semibold text-white mb-4">Today vs Yesterday</h3>
      ...
      <div className="mt-4 p-3 bg-white/10 rounded-lg">
        <p className="text-white/80 text-sm text-center">
          {data.temperature.current > data.temperature.previous + 5 
            ? "🔥 Warmer day - consider earlier morning run"
            : data.windSpeed.current > data.windSpeed.previous + 3
            ? "💨 Windier conditions - check your route"
            : data.cloudCoverage.current < data.cloudCoverage.previous - 20
            ? "☀️ Clearer skies - great for running!"
            : "🏃‍♂️ Good conditions for your run"
          }
        </p>
      </div>
    </div>
